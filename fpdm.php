...
if (!call_user_func_array('class_exists', $__tmp)) {
    ...
    class FPDM
    {
        ...
        public $pdf_source = '';      //string: full pathname to the input pdf , a form file
        public $fdf_source = '';	  //string: full pathname to the input fdf , a form data file
        public $pdf_output = '';      //string: full pathname to the resulting filled pdf

        public $pdf_entries = array();     //array: Holds the content of the pdf file as array
        public $fdf_content = '';     //string: holds the content of the fdf file
        public $fdf_parse_needed = false;//boolean: false will use $fields data else extract data from fdf content
        public $value_entries = array();   //array: a map of values to faliclitate access and changes

        public $positions = array(); 	  //array, stores what object id is at a given position n ($positions[n]=<obj_id>)

        public $offsets = array();   	  //array of offsets for objects, index is the object's id, starting at 1
        public $pointer = 0;   	  //integer, Current line position in the pdf file during the parsing

        public $shifts = array();    	  //array, Shifts of objects in the order positions they appear in the pdf, starting at 0.
        public $shift = 0;     	  //integer, Global shift file size due to object values size changes

        public $streams = '';         //Holds streams configuration found during parsing
        public $streams_filter = '';  //Regexp to decode filter streams

        public $safe_mode = false;       //boolean, if set, ignore previous offsets do no calculations for the new xref table, seek pos directly in file
        public $check_mode = false;      //boolean, Use this to track offset calculations errors in corrupteds pdfs files for sample
        public $halt_mode = false; 	  //if true, stops when offset error is encountered

        public $info = array(); 			  //array, holds the info properties
        public $fields = array(); 		  //array that holds fields-Data parsed from FDF

        public $verbose = false;         //boolean ,  a debug flag to decide whether or not to show internal process
        public $verbose_level = 1;   //integer default is 1 and if greater than 3, shows internal parsing as well

        public $support = ''; 		  //string set to 'native' for fpdm or 'pdftk' for pdf toolkit
        public $flatten_mode = false;	  //if true, flatten field data as text and remove form fields (NOT YET SUPPORTED BY FPDM)
        public $compress_mode = false;   //boolean , pdftk feature only to compress streams
        public $uncompress_mode = false; //boolean pdftk feature only to uncompress streams
        public $security = array();        //Array holding securtity settings
                              //(password owner nad user,  encrypt (set to 40 or 128 or 0), allow <permissions>] see pdfk help

        public $needAppearancesTrue = false;	//boolean, indicates if /NeedAppearances is already set to true
        public $isUTF8 = false;				//boolean (true for UTF-8, false for ISO-8859-1)
...
//							$this->set_field_value("tooltip",$name,$value);
        ...
        **/
        ...
        public function _set_field_value2($line, $value, $append)
        {
            $CurLine=$this->pdf_entries[$line];
            $OldLen=strlen($CurLine);
        ...
        *@param string $value the new value to set
        **/
        public function set_field_value($type, $name, $value)
        {
            //------------------------------------
            $verbose_set=($this->verbose&&($this->verbose_level>1));
            ...
            if (isset($this->value_entries["$name"])) {
                ...
                } elseif (isset($this->value_entries["$name"]['infos']['checkbox_state'])) { //FIX: set checkbox value
        ...
        **/
        ...
        //FIX: checkbox
        /**
        *Changes the checkbox value of a field property, inline.
        *
        *@param string $name name of the field annotation to change the value
        *@param string $value the new value to set
        *@return int offset_shift the size variation
...
//									if(!array_key_exists("default",$values)) $this->Error("Cannot find default value (/DV) for field $name");
                                ...
                                if (array_key_exists("length", $stream)) { //length is mandatory
                                    ...
                                    */
                                    ...
                                    $match=array();
                                    if (($ap_d_yes=='')||($ap_d_no=='')||($as=='')||($type=='')||($subtype=='')||($name=="")) {
                                        //FIX: read checkbox
                                        if (!$ap_line && '/AP' == substr($CurLine, 0, 3)) {
                                            ...
                                            $object["infos"]["checkbox_state"]=$as; //Keep a track
                                            $object["infos"]["checkbox_state_line"]=$Counter; //Keep a track
                                        }
                                        //ENDFIX
                                        ...
                                        if (($name=="")&&preg_match("/^\/T\s?\((.+)\)\s*$/", $this->_protectContentValues($CurLine), $match)) {
                                            ...
                                            //FIX: convert object names to utf-8
                                            // don't use utf8_encode($name), it's only core function since php 7.2
